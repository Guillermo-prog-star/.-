...
**Amendment (2026-09-15):** se agrega Decisión 5 — una capa "Bruno-Feynman" (arte de la memoria de Giordano Bruno + niveles N1–N5 de apropiación) sobre el mismo instrumento ya diferido, sin construir nada ni adelantar ningún disparador. Verificado contra el código antes de escribir — ver bullet nuevo bajo "Verificado".
...
- **No hay rastro de un "método Bruno" en el código.** Búsqueda dirigida de `loci|imagen.mental|palacio.de.la.memoria|mnemotecni|ars memoriae` en todo el repo: los 19 archivos que matchean son falsos positivos de la subcadena `loci` dentro de otras palabras (ej. `velocidad`), verificado leyendo el contexto real de una de las coincidencias. Confirma que no hay nada parcialmente construido que esta capa deba reconciliar.
...
### Decisión 5 — capa "Bruno-Feynman" se registra como enriquecimiento del mismo diseño diferido, sin construir nada (amendment 2026-09-15)

Una sesión posterior sobre el arte de la memoria de Giordano Bruno (*De umbris idearum*, 1582 — loci, imagines agentes, ruedas combinatorias influidas por Llull) propone añadir, antes del paso "explicar" de la ficha genérica de 14 campos, un paso de **representación**: convertir el fenómeno familiar abstracto en una imagen espacial concreta (lugar + imagen + acción) antes de intentar explicarlo con palabras propias. La misma sesión propone leer la progresión de apropiación de una ficha en cinco niveles: **N1 Reconozco → N2 Recuerdo → N3 Explico → N4 Experimento → N5 Transfiero**.

Esto no es territorio nuevo: es una variación de contenido sobre el mismo instrumento que la Decisión 1 de este ADR ya registró (ficha de 14 campos + catálogo de 300 microexperimentos, 60 mecanismos × 5 contextos) y que la Decisión 2 ya congeló sin construir. Se deja constancia aquí, junto al resto del diseño EF-Feynman, con las mismas condiciones:

- **No se modifica** el catálogo real (`Integrity_Family_300_Microexperimentos_Filosoficos_Feynman.docx`, fuera del repo) — la capa Bruno queda como nota de diseño para cuando (si alguna vez) se retome la línea completa, no como cambio a un documento que hoy no vive en control de versiones.
- **No se agrega** ningún campo N1–N5 a `TaskEvidence`, `ChecklistItem` ni ninguna entidad existente — repetiría exactamente el error que la Decisión 3 de este ADR ya previno (crear estructura nueva en vez de extender lo existente cuando ni siquiera lo existente tiene disparador todavía).
- **Advertencia explícita contra sobrecarga cognitiva:** la propia sesión que propuso esto señaló su límite — un sistema simbólico de representación puede producir sensación de dominio sin comprensión causal (recordar que "X se asocia con Y" no es explicar "por qué X produce Y"). Si esta capa se retoma alguna vez, el paso de representación es un puente hacia "explicar" (Feynman), nunca un sustituto — la ficha no se considera completa en N1/N2 sin pasar por N3.
...
4. [x] Registrar la capa Bruno-Feynman (representación espacial + niveles N1–N5) como enriquecimiento del mismo diseño diferido, sin construir nada (Decisión 5, amendment 2026-09-15).
